<?php
/**
 * Debug script to check why related products aren't showing
 * This script will help identify the issue with related products display
 */
...
echo "=== WRP Related Products Display Debug ===\n\n";
...
echo "Test 2: Check Cache Status\n";
try {
    ...
    echo "✗ Failed to get cache status: " . $e->getMessage() . "\n";
}
echo "\n";

// Test 3: Test Getting Related Products
echo "Test 3: Test Getting Related Products\n";
try {
    $product_id = 1;
    echo "Testing related products for product ID: $product_id\n";
    ...
    echo "✗ Failed to get related products: " . $e->getMessage() . "\n";
    echo "Stack trace:\n" . $e->getTraceAsString() . "\n";
}
echo "\n";

// Test 4: Test Display Related Products
echo "Test 4: Test Display Related Products\n";
try {
    $product_id = 1;
...
// Test 5: Test Template File Loading
echo "Test 5: Test Template File Loading\n";
try {
...
// Test 6: Test Auto Display Settings
echo "Test 6: Test Auto Display Settings\n";
try {
    $auto_display = get_option('wrp_auto_display', true);
    $display_position = get_option('wrp_display_position', 'after_content');
    echo "✓ Auto display: " . ($auto_display ? 'Enabled' : 'Disabled') . "\n";
    echo "✓ Display position: $display_position\n";
} catch (Exception $e) {
    echo "✗ Failed to get auto display settings: " . $e->getMessage() . "\n";
}
echo "\n";

echo "=== Debug Complete ===\n";
echo "If all tests pass, the issue might be:\n";
echo "1. WordPress hooks not firing correctly\n";
echo "2. Theme or plugin conflicts\n";
echo "3. CSS hiding the related products\n";
echo "4. JavaScript errors preventing display\n";
?>