...
class WRP_Core {
    ...
    private function register_hooks() {
        ...
        // Add theme-specific hooks for better compatibility
        add_action( 'woocommerce_after_single_product', array( $this, 'maybe_display_related_products_theme_fallback' ), 20 );
        
        // Add additional hooks for themes that might override standard WooCommerce hooks
        add_action( 'wp_footer', array( $this, 'maybe_display_related_products_fallback' ), 100 );
    ...
    /**
     * Theme fallback method - display related products after single product
     */
    public function maybe_display_related_products_theme_fallback() {
        // Only run if auto display is enabled and we haven't displayed related products yet
        if ( ! wrp_get_option( 'auto_display', true ) ) {
            return;
        }
        
        // Check if we've already displayed related products
        global $wrp_related_products_displayed;
        if ( $wrp_related_products_displayed ) {
            return;
        }
        ...
        $this->auto_display_related_products();
        $wrp_related_products_displayed = true;
    }

    /**
     * Ultimate fallback method - display related products in footer if not shown yet
     */
    public function maybe_display_related_products_fallback() {
        // Only run if auto display is enabled and we haven't displayed related products yet
        if ( ! wrp_get_option( 'auto_display', true ) ) {
            return;
        }
        ...
        // Check if we're in the main content area (not in sidebars or other areas)
        if ( ! is_main_query() ) {
            return;
        }
        
        // Display related products with a wrapper for positioning
        echo '<div class="wrp-fallback-container" style="margin-top: 40px; clear: both;">';
        $this->auto_display_related_products();
        echo '</div>';
        $wrp_related_products_displayed = true;
    }
    ...
    public function auto_display_related_products() {
        ...
        if ( ! $product ) {
            error_log('WRP Core: No global product found in auto_display_related_products');
            return;
        }
        
        $product_id = $product->get_id();
        error_log("WRP Core: Auto displaying related products for product ID: $product_id");
        
        // Set global flag to prevent duplicate display
        global $wrp_related_products_displayed;
        $wrp_related_products_displayed = true;
        
        // Get theme-specific display settings
        $theme_compatibility = $this->get_theme_compatibility_settings();
        
        // Apply theme-specific wrapper classes
        $wrapper_classes = $theme_compatibility['wrapper_classes'];
        
        // Add theme-specific wrapper
        echo '<div class="' . esc_attr( implode( ' ', $wrapper_classes ) ) . '">';
        
        $this->display_related_products( $product_id );
        
        echo '</div>';
    }

    /**
     * Get theme-specific compatibility settings
     *
     * @return array
     */
    private function get_theme_compatibility_settings() {
        $current_theme = wp_get_theme();
        $theme_name = strtolower( $current_theme->get( 'Name' ) );
        
        $settings = array(
            'wrapper_classes' => array( 'wrp-related-products-container' ),
            'priority' => array(
                'before_content' => 30,
                'after_content' => 15,
                'after_cart' => 10,
        ...
        // Woodmart theme specific settings
        if ( strpos( $theme_name, 'woodmart' ) !== false || strpos( $theme_name, 'wood' ) !== false ) {
            $settings['wrapper_classes'][] = 'wrp-woodmart-compatible';
            $settings['priority']['after_content'] = 25; // Higher priority for Woodmart
            error_log('WRP Core: Woodmart theme detected, applying compatibility settings');
        }
        
        // Twenty Twenty theme specific settings
        if ( strpos( $theme_name, 'twenty twenty' ) !== false || strpos( $theme_name, 'twentytwenty' ) !== false ) {
            $settings['wrapper_classes'][] = 'wrp-twentytwenty-compatible';
            $settings['priority']['after_content'] = 10; // Lower priority for Twenty Twenty
            error_log('WRP Core: Twenty Twenty theme detected, applying compatibility settings');
        }
        
        // Store theme detection for debugging
        set_transient( 'wrp_current_theme', $theme_name, 3600 );
    ...
    public function get_related_products( $product_id, $args = array() ) {
        if ( ! $product_id || ! is_numeric( $product_id ) ) {
            error_log("WRP Core: Invalid product ID: $product_id");
            return false;
        }

        $product = wc_get_product( $product_id );
        if ( ! $product ) {
            error_log("WRP Core: Product not found for ID: $product_id");
            return false;
        }
        ...
        try {
            ...
            // Update default threshold and limit for better results
            if (!isset($args['threshold']) || $args['threshold'] === $this->default_options['threshold']) {
                $args['threshold'] = 1.5; // Lower threshold for more results
            }
            if (!isset($args['limit']) || $args['limit'] === $this->default_options['limit']) {
                $args['limit'] = 12; // Show more products like YARPP
            }
            
            error_log("WRP Core: Getting related products for product ID: $product_id with args: " . json_encode($args));

            // Try enhanced cache first
            $enhanced_cache = new WRP_Enhanced_Cache($this);
            $related_ids = $enhanced_cache->get_related_ids( $product_id, $args );
            error_log("WRP Core: Enhanced cache returned " . count($related_ids) . " related IDs");
            ...
            error_log("WRP Core: Enhanced cache failed: " . $e->getMessage() . ", trying original cache");
        }

        // Try original cache system as fallback
        try {
            // Parse arguments
            $args = wp_parse_args( $args, $this->default_options );
            
            error_log("WRP Core: Trying original cache for product ID: $product_id");

            // Try to get related product IDs from cache or compute
            $related_ids = $this->cache->get_related_ids( $product_id, $args );
            error_log("WRP Core: Original cache returned " . count($related_ids) . " related IDs");
            ...
            error_log("WRP Core: Original cache failed: " . $e->getMessage() . ", trying YARPP cache");
        }

        // Try with lower threshold if no products found
        if (empty($related_ids)) {
            try {
                error_log("WRP Core: No related products found with original threshold, trying with lower threshold");
                $low_threshold_args = $args;
                $low_threshold_args['threshold'] = 0.1; // Very low threshold
                
                $related_ids = $this->cache->get_related_ids( $product_id, $low_threshold_args );
                error_log("WRP Core: Low threshold cache returned " . count($related_ids) . " related IDs");
                
                if (!empty($related_ids)) {
                    return $this->convert_ids_to_products($related_ids, $low_threshold_args);
                }
            } catch (Exception $e) {
                error_log("WRP Core: Low threshold cache failed: " . $e->getMessage());
            }
        }
     ...
     * @return array
     */
    private function convert_ids_to_products($related_ids, $args) {
        ...
        foreach ( $related_ids as $id ) {
            ...
            if ( $related_product ) {
                ...
                if ( ! $args['include_out_of_stock'] && ! $related_product->is_in_stock() ) {
                    error_log("WRP Core: Skipping out of stock product ID: $id");
                    continue;
                }
                $related_products[] = $related_product;
                error_log("WRP Core: Added product ID: $id to related products");
            } else {
                error_log("WRP Core: Could not get product for ID: $id");
            }
        }

        error_log("WRP Core: Returning " . count($related_products) . " related products");
    ...
    public function display_related_products( $product_id, $args = array(), $echo = true ) {
        // Parse arguments with defaults that are more permissive for display
        $default_args = wp_parse_args( $args, $this->default_options );
        
        // For display, we want to include out-of-stock products to show more options
        $display_args = $default_args;
        $display_args['include_out_of_stock'] = true; // Show out-of-stock products in display
        
        $related_products = $this->get_related_products( $product_id, $display_args );
    ...
    public function on_product_save( $post_id, $post, $update ) {
        ...
        // Clear cache for this product using YARPP-style cache
        try {
            $yarpp_cache = new WRP_YARPP_Cache();
            $yarpp_cache->invalidate_product( $post_id );
        } catch (Exception $e) {
            error_log("WRP Core: Failed to invalidate YARPP cache for product $post_id: " . $e->getMessage());
        }
        ...
        try {
            ...
            error_log("WRP Core: Failed to get related references for product $post_id: " . $e->getMessage());
        }
    }
    ...
    public function on_stock_change( $product ) {
        if ( ! wrp_get_option( 'include_out_of_stock', false ) ) {
            ...
            // Clear cache using YARPP-style cache
            try {
                $yarpp_cache = new WRP_YARPP_Cache();
                $yarpp_cache->invalidate_product( $product_id );
            } catch (Exception $e) {
                error_log("WRP Core: Failed to invalidate YARPP cache for product $product_id: " . $e->getMessage());
            }
